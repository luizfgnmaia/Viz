...
author: "Fundação Getulio Vargas <br> Mestrado em Modelagem Matemática <br> Prof. Asla Sá <br> Aluno: Luiz Fernando Maia"
...
No entanto, um problema que surgiu foi que a api gratuita do Tweeter aparentemente não retorna tweets de maneira constante ao decorrer da partida. Segue um exemplo em que temos tweets apenas para alguns intervalos de tempo do jogo.
```{r, message = FALSE}
...
origem = tibble(speaker = personagens, origem = c("water", "air", "water", "fire", "fire", "earth", "fire", "fire", "earth", "water", "air", "fire", "fire", "earth", "water", "earth", "earth", "fire"))
origem
```
...
tibble(spearman = cor_rating, speaker = names(cor_rating)) %>%
  inner_join(origem, by = "speaker") %>%
  mutate(speaker = reorder(speaker, spearman)) %>%
  ggplot(aes(speaker, spearman, fill = origem)) +
  geom_bar(stat = "identity") +
  xlab("Personagem") +
...
Posteriormente, considerei apenas as palavras referentes aos nomes dos 18 personagens selecionados e ilustrei a quantidade de vezes que um personagem diz o nome de outro.
```{r}
tmp6 = tfidf %>%
